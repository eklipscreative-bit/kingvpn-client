...
class WindowsBuilder(Builder):
    ...
    def _target_architecture() -> str:
        configured = os.environ.get("KINGVPN_WINDOWS_ARCH")
        if configured in ("x64", "arm64"):
            return configured
        if configured:
            raise ValueError("KINGVPN_WINDOWS_ARCH must be x64 or arm64")
        if is_amd64():
            return "x64"
        if is_arm64():
            return "arm64"
    ...
    def _release_bundle(self) -> Path:
        ...
        for name in (
            f"{_APP_EXE}", "flutter_windows.dll", *_RUNTIME_FILES,
    ...
    def package_msix(self):
        ...
        local_development = os.environ.get("KINGVPN_DEV_SIGN") == "1"
        package_with_vcore(
            os.path.join(self.output_dir, f"{output_name}.msix"),
            local_development=local_development,
            certificate_thumbprint=os.environ.get("KINGVPN_DEV_CERT_THUMBPRINT"),
            certificate_path=os.environ.get("KINGVPN_DEV_CERT_PATH"),
            certificate_password=os.environ.get("KINGVPN_DEV_CERT_PASSWORD"),
            development_publisher=os.environ.get("KINGVPN_DEV_PUBLISHER"),
        )

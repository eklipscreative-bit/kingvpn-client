...
_GITHUB_RELEASE_PACKAGES = {
    ...
    ("linux", "aarch64", None): (
        "linux-arm64/KingVPN-linux-aarch64.zip",
        "linux-arm64/KingVPN-linux-aarch64.deb",
    ),
    ("windows", "x64", "exe"): (
        "windows-x64/KingVPN-windows-amd64.exe",
        "windows-x64/KingVPN-windows-amd64.zip",
    ),
    ("windows", "arm64", "exe"): (
        "windows-arm64/KingVPN-windows-arm64.exe",
        "windows-arm64/KingVPN-windows-arm64.zip",
    ),
}
...
def begin_build(builder, target: str) -> dict:
    ...
    sources = {
        ...
        "libXray": source_revision(
            ...
            os.environ.get("KINGVPN_LIBXRAY_SHA"),
        ),
    }
    source_paths = {
        "app": root,
    ...
    if target == "windows":
        ...
        sources["VCore"] = source_revision(
            source_paths["VCore"], os.environ.get("KINGVPN_VCORE_SHA"),
        )
    return {
        "formatVersion": 1,
        "target": target,
...
def finish_build(builder, receipt: dict) -> Path:
    ...
    for path in lock_files:
        if path.is_file():
            name = (f"KingVPN/{path.relative_to(root).as_posix()}" if path.is_relative_to(root)
    ...
    for path in native_paths:
        for file in path.rglob("*") if path.is_dir() else [path]:
            if file.is_file():
                files[f"KingVPN/{file.resolve().relative_to(root.resolve()).as_posix()}"] = sha256(file)

    patterns = {
        "ios": ("KingVPN-ios*.ipa",),
        "macos": ("*.pkg",),
        "macos_se": ("KingVPN-macos-universal*.zip",),
        "android": ("KingVPN-android-universal*.apk", "app-release.aab"),
        "linux": (f"KingVPN-{builder.builder.package_suffix}.zip",
                  f"KingVPN-{builder.builder.package_suffix}.deb"),
    }
    if target == "windows":
        extensions = ("msix",) if receipt["windowsMode"] == "msix" else ("exe", "zip")
        patterns["windows"] = tuple(f"KingVPN-{builder.builder.package_suffix}.{ext}"
    ...
    receipt.update({
        ...
        "windowsToolchain": {key: os.environ.get(key) for key in (
            "VCToolsVersion", "WindowsSDKVersion", "KINGVPN_LLVM_MINGW_RELEASE",
            "KINGVPN_LLVM_MINGW_SHA256",
...
def verify_release(artifacts: Path, run: dict, *, tag: str | None = None,
    ...
    if windows_only:
        ...
        if {path.name for path in packages} != {
            "KingVPN-windows-amd64.msix", "KingVPN-windows-arm64.msix",
        } or len(packages) != 2:
    ...
    if windows_only:
        ...
        for architecture, name in (("x64", "KingVPN-windows-amd64.msix"),
                                   ("arm64", "KingVPN-windows-arm64.msix")):

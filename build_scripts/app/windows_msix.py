    ...
    *,
    ...
    if local_development:
        if not development_publisher:
            raise ValueError("KINGVPN_DEV_PUBLISHER is required")
        identity.set("Name", "KingVPN.Dev")
    ...
    if local_development:
        ...
        if certificate_thumbprint:
            ...
            if not re.fullmatch(r"[0-9A-F]{40}", certificate_thumbprint):
                raise ValueError(
                    "KINGVPN_DEV_CERT_THUMBPRINT must contain exactly "
            ...
            raise ValueError(
                "KINGVPN_DEV_CERT_THUMBPRINT or KINGVPN_DEV_CERT_PATH and "
                "KINGVPN_DEV_CERT_PASSWORD are required"
            )
    makeappx = _sdk_tool("makeappx.exe")
    with tempfile.TemporaryDirectory(prefix="kingvpn-msix-") as temporary:
        stage = os.path.join(temporary, "stage")
        rebuilt = os.path.join(temporary, "KingVPN.msix")

...
class WindowsPackagingTest(unittest.TestCase):
    ...
    def _bundle(self):
        ...
        for name in ("KingVPNClient.exe", "flutter_windows.dll", *_RUNTIME_FILES, *runtime_files):
            ...
            with self.subTest(architecture=target):
                ...
                def package(*args, **kwargs):
                    ...
                    for extension in ("exe", "zip"):
                        (dist / f"KingVPN-windows-{package_arch}.{extension}").write_bytes(
                ...
                fastforge.assert_called_once_with(
                    ...
                    arguments=(
                        ...
                        "--artifact-name", f"KingVPN-windows-{package_arch}." + "{{ext}}",
                    ),
                    env={"PROCESSOR_ARCHITECTURE": processor_arch},
                )
                for extension in ("exe", "zip"):
                    self.assertEqual(
                        (Path(self.builder.output_dir) / f"KingVPN-windows-{package_arch}.{extension}").read_bytes(),
                        f"Fastforge {target} {extension}".encode(),
                    )
    ...
    def test_fastforge_requires_both_outputs_for_the_current_version(self):
        ...
        for version, extensions in (("old-version", ("exe", "zip")), ("26.7.3", ("exe",))):
            ...
            for extension in extensions:
                (directory / f"KingVPN-windows-amd64.{extension}").write_bytes(b"fixture")
    ...
    def test_fastforge_rejects_missing_visual_cpp_runtime_before_collecting_packages(self):
        ...
        for architecture, package_arch, required_files in (
            ...
            dist.mkdir(parents=True, exist_ok=True)
            for extension in ("exe", "zip"):
                (dist / f"KingVPN-windows-{package_arch}.{extension}").write_bytes(b"fixture")
    ...
    def test_installer_preserves_released_identity_and_scopes_cleanup(self):
        ...
        self.assertIn("executable_name: KingVPNClient.exe", config)
    ...
    def test_winget_workflow_publishes_stable_exe_installers_only(self):
        ...
        self.assertIn("identifier: KingVPN.KingVPNClient", workflow)
        ...
        for name in ("KingVPN-windows-amd64.exe", "KingVPN-windows-arm64.exe"):
            self.assertRegex(name, pattern)
            self.assertIn(name, workflow)
        for name in ("KingVPN-windows-amd64.zip", "KingVPN-windows-arm64.msix",
                     "OneXrayCore.exe", "KingVPN-windows-amd64.exe.sig"):
    ...
    def test_arm64_msix_uses_arm64_architecture(self):
        ...
        self.assertEqual(command[-1], "KingVPN-windows-arm64")
    ...
    def test_target_architecture_prefers_workflow_setting(self):
        with patch.dict(os.environ, {"KINGVPN_WINDOWS_ARCH": "arm64"}):
    ...
    def test_local_signing_requires_certificate_and_publisher(self):
        with self.assertRaises(ValueError):
            package_with_vcore(
                ...
                development_publisher="CN=KingVPN Development",
            )
        ...
        for local_development in (False, True):
            with self.subTest(local_development=local_development):
                ...
                self.assertEqual(
                    ...
                    "KingVPN.Dev" if local_development else "KingVPN.KingVPNClient",
                )
                self.assertEqual(
                    identity.attrib["Publisher"],
                    "CN=KingVPN Development" if local_development else "CN=Store",
                )
                ...
                self.assertEqual(application.attrib["Executable"], "KingVPNClient.exe")
                self.assertFalse(
...
<Package xmlns="{_FOUNDATION}"
 ...
 <Identity Name="KingVPN.KingVPNClient" Publisher="CN=Store" Version="1.0.0.0" ProcessorArchitecture="x64" />
 <Capabilities>
 ...
 <Applications>
  <Application Id="OneXray" Executable="KingVPNClient.exe" EntryPoint="Windows.FullTrustApplication">
   <Extensions>
    <uap:Extension Category="windows.protocol">
     <uap:Protocol Name="onexray" />
    </uap:Extension>
    <desktop:Extension Category="windows.startupTask" Executable="KingVPNClient.exe" EntryPoint="Windows.FullTrustApplication">

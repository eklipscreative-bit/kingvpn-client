...
class ProvenanceTest(unittest.TestCase):
    ...
    def receipt(self, architecture, *, target="windows", paths=None, mode="msix"):
        ...
        paths = paths or [f"windows-store-{architecture}/KingVPN-windows-{suffix}.msix"]
    ...
    def public_receipts(self, build_target):
        ...
        for target, architecture, paths in (
            ...
            ("windows", "x64", ["windows-x64/KingVPN-windows-amd64.exe",
                                 "windows-x64/KingVPN-windows-amd64.zip"]),
            ("windows", "arm64", ["windows-arm64/KingVPN-windows-arm64.exe",
    ...
    def test_receipt_records_actual_files_and_keeps_other_platform_packages_out(self):
        ...
        package = output / "KingVPN-linux-x86_64.zip"
        package.write_bytes(b"fixture package")
        (output / "KingVPN-windows-amd64.msix").write_bytes(b"other target")
        builder = SimpleNamespace(
        ...
        self.assertEqual(receipt["fileSha256"]["KingVPN/assets/geodata/regions.json"], sha256(regions))
        self.assertEqual(receipt["fileSha256"]["KingVPN/linux/app/libXray.so"], sha256(library))
    ...
    def test_windows_receipt_records_only_the_built_mode(self):
        ...
        for extension in ("msix", "exe", "zip"):
            (output / f"KingVPN-windows-amd64.{extension}").write_bytes(extension.encode())
        builder = SimpleNamespace(
            root_dir=str(root), output_dir=str(output), workspace_dir=str(self.artifacts),
        ...
        for mode, extensions in (("exe", ("exe", "zip")), ("msix", ("msix",))):
            ...
            self.assertEqual(set(receipt["packages"]), {f"KingVPN-windows-amd64.{ext}" for ext in extensions})
            ...
            self.assertIn("KingVPN/windows/app/wintun.dll", receipt["fileSha256"])

...
class BuilderTest(unittest.TestCase):
    ...
    def test_windows_cli_defaults_to_exe_and_selects_msix_explicitly(self):
        for options, mode in (([], "exe"), (["--windows-mode", "msix"], "msix")):
            with mock.patch.object(sys, "argv", ["build", "KingVPN", "windows", *options]), mock.patch("main.FlutterBuilder") as builder:
                main()
                self.assertEqual(builder.call_args.kwargs["windows_mode"], mode)
                builder.return_value.build.assert_called_once()
        with mock.patch.object(sys, "argv", ["build", "KingVPN", "linux", "--windows-mode", "exe"]), mock.patch("main.FlutterBuilder") as builder, mock.patch("sys.stderr"):
    ...
    def test_flutter_windows_build_and_packager_share_one_mode(self):
        for mode in ("exe", "msix"):
            with mock.patch.dict("os.environ", {"BUILD_NUMBER": "1", "KINGVPN_WINDOWS_ARCH": "x64"}):
                builder = FlutterBuilder("KingVPN", "windows", str(self.root_dir / "build_scripts"), windows_mode=mode)

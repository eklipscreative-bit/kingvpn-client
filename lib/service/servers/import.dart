...
import 'package:kingvpn/core/errors/failure.dart';

import 'package:file_picker/file_picker.dart';
import 'package:flutter/services.dart';
import 'package:image_picker/image_picker.dart';
import 'package:kingvpn/core/db/database/database.dart';
import 'package:kingvpn/core/network/client.dart';
import 'package:kingvpn/core/pigeon/host_api.dart';
import 'package:kingvpn/service/shared/db/config_writer.dart';
import 'package:kingvpn/service/connect/raw/editor.dart';
import 'package:kingvpn/service/advanced/xray/geodata/service.dart';
import 'package:kingvpn/service/advanced/xray/geodata/model.dart';
import 'package:kingvpn/service/advanced/xray/geodata/validator.dart';
import 'package:kingvpn/service/shared/ping/service.dart';
import 'package:kingvpn/service/shared/xray/validation.dart';
...
import 'package:kingvpn/service/shared/share/configuration_transfer.dart';
import 'package:kingvpn/service/connect/routing/custom/service.dart';
import 'package:kingvpn/service/connect/routing/custom/document.dart';
import 'package:kingvpn/service/shared/in_flight_operations.dart';
import 'package:kingvpn/service/shared/share/xray_share_reader.dart';
import 'package:kingvpn/service/servers/subscription/model.dart';
import 'package:kingvpn/service/servers/subscription/service.dart';
import 'package:kingvpn/service/servers/subscription/validator.dart';
import 'package:kingvpn/service/servers/outbound/map.dart';
import 'package:kingvpn/service/servers/outbound/state_db.dart';
import 'package:kingvpn/service/connect/raw/db.dart';
import 'package:kingvpn/service/connect/raw/validator.dart';
...
class ServerImportResult {
  ...
  final List<KingVpnGeoDataLink> failedGeoData;
...
class ServerImportPreview {
  ...
  final List<KingVpnGeoDataLink> geoData;
  ...
  ServerImportPreview(
    ...
    Iterable<KingVpnGeoDataLink> geoData = const [],
...
class ServerImportDetection {
  final List<KingVpnSubscriptionLink> subscriptions;
...
class ServerImportService {
  ...
  final Future<SubscriptionInsertResult> Function(KingVpnSubscriptionLink)
  _subscribe;
  final Future<bool> Function(KingVpnGeoDataLink) _validateGeoData;
  final Future<bool> Function(KingVpnGeoDataLink) _writeGeoData;
  ...
  ServerImportService({
    ...
    Future<SubscriptionInsertResult> Function(KingVpnSubscriptionLink)?
    subscribe,
    Future<bool> Function(KingVpnGeoDataLink)? validateGeoData,
    Future<bool> Function(KingVpnGeoDataLink)? writeGeoData,
  }) : _database = database,
  ...
  ServerImportDetection detect(String text) {
    ...
    final subscriptions = <KingVpnSubscriptionLink>[];
    final local = <String>[];
    for (final line in text.split('\n')) {
      final link = singleLink(line);
      if (link is KingVpnSubscriptionLink) {
  ...
  Future<List<ServerSubscriptionImport>> importSubscriptions(
    List<KingVpnSubscriptionLink> links,
  ...
  Future<ServerImportPreview> preview(
    ...
    if (!manual) {
      ...
      final geoData = <KingVpnGeoDataLink>[];
      final other = <String>[];
      final configurations = <ConfigurationContent>[];
      final parsedLinks = [
        for (final line in text.split('\n'))
          if (Uri.tryParse(line.trim()) case final uri?)
            KingVpnAppLinkParser.parse(uri),
      ];
      final usedGeoData = <KingVpnGeoDataLink>{};
      for (final line in text.split('\n')) {
        final uri = Uri.tryParse(line.trim());
        final link = uri == null ? null : KingVpnAppLinkParser.parse(uri);
        if (link == null) {
          if (uri?.scheme.toLowerCase() == KingVpnAppLinkParser.scheme) {
            continue;
          }
          other.add(line);
          continue;
        ...
        try {
          if (link is KingVpnConfigLink &&
              link.type == KingVpnConfigLinkType.outbound) {
          ...
          } else if (link is KingVpnConfigLink &&
              (link.type == KingVpnConfigLinkType.raw ||
                  link.type == KingVpnConfigLinkType.custom)) {
            final kind = link.type == KingVpnConfigLinkType.raw
            ...
            if (kind == ConfigurationKind.raw) {
              ...
              for (final data in parsedLinks.whereType<KingVpnGeoDataLink>()) {
                if (references.containsKey(data.name) ||
                    references.containsKey('${data.name}.dat')) {
                  usedGeoData.add(data);
                  dependencies.add(
                    Uri(
                      scheme: KingVpnAppLinkParser.scheme,
                      host: KingVpnAppLinkParser.host,
                      path: KingVpnAppLinkParser.geoDataPath,
                      queryParameters: {
                        'type': data.type.name,
                        'url': data.url,
                      },
          ...
          } else if (link is KingVpnGeoDataLink) {
            geoData.add(link);
          } else {
      ...
      final standalone = <KingVpnGeoDataLink>[];
  ...
  Future<ServerImportPreview> _configurationPreview(
    ...
    List<KingVpnGeoDataLink> standalone,
  ) async {
  ...
  Future<ServerImportResult> _commitPreview(ServerImportPreview preview) async {
    ...
    var geoDataCount = 0;
    final failures = <KingVpnGeoDataLink>[];
  ...
  static KingVpnAppLink? singleLink(String text) {
    final uri = Uri.tryParse(text.trim());
    if (uri == null || text.trim().contains('\n')) return null;
    final link = KingVpnAppLinkParser.parse(uri);
    if (link != null) return link;
    if (!NetClient.isHttpsDownloadUri(uri)) return null;
    return KingVpnSubscriptionLink(
      name: uri.fragment,

...
import 'package:kingvpn/pages/shared/share/controller.dart';
...
import 'package:kingvpn/service/shared/share/outgoing_share.dart';
  ...
  for (final kind in ConfigurationKind.values) {
    for (final copy in [false, true]) {
      testWidgets('${kind.name} tools share from their button, copy=$copy', (
        ...
        await tester.pumpAndSettle();
        final link =
            KingVpnAppLinkParser.parse(Uri.parse(links))! as KingVpnConfigLink;
        expect(
          link.type,
          kind == ConfigurationKind.raw
              ? KingVpnConfigLinkType.raw
              : KingVpnConfigLinkType.custom,
        );

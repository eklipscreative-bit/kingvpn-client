...
void main() {
  ...
  test('Raw source and independent source links round trip without formatting or double encoding', () async {
    ...
    expect(links.first, isA<KingVpnGeoDataLink>());
    expect((links.last as KingVpnConfigLink).xrayJson, source);
  ...
  test(
    ...
    () async {
      ...
      final link =
          KingVpnAppLinkParser.parse(Uri.parse(text))! as KingVpnConfigLink;
      expect(link.type, KingVpnConfigLinkType.custom);
  ...
  test('type=custom rejects unsupported rules without falling back to Raw or node import', () async {
    ...
    final link = KingVpnAppLinkGenerator.configurationText(

...
import 'package:kingvpn/core/db/database/enum.dart';
...
import 'package:kingvpn/service/shared/share/app_link_share_service.dart';

void main() {
  group('config links', () {
    for (final entry in const <CoreConfigType, KingVpnConfigLinkType>{
      CoreConfigType.outbound: KingVpnConfigLinkType.outbound,
      CoreConfigType.raw: KingVpnConfigLinkType.raw,
    }.entries) {
      test('generates and parses ${entry.value.wireName}', () {
        final xrayJson = entry.key == CoreConfigType.outbound
            ...
            : '{"name":"Example"}';
        final uri = KingVpnAppLinkGenerator.config(
          _config(
            type: entry.key.name,
        ...
        final link =
            KingVpnAppLinkParser.parse(generatedUri)! as KingVpnConfigLink;
    ...
    test('Raw link preserves unprojected fields and original JSON text', () {
      ...
      final link = KingVpnAppLinkParser.parse(uri!)! as KingVpnConfigLink;

      expect(link.type, KingVpnConfigLinkType.raw);
      expect(link.name, 'Shared Config');
    ...
    test('rejects unsupported types and invalid data', () {
      expect(
        KingVpnAppLinkGenerator.config(_config(type: 'unsupported')),
        isNull,
      );
      expect(
        KingVpnAppLinkGenerator.config(
    ...
    test('places referenced GeoData links before the config link', () async {
      ...
      final service = KingVpnAppLinkShareService(
      ...
      final links = text!
          .split('\n')
          .map(Uri.parse)
          .map(KingVpnAppLinkParser.parse)
          .whereType<KingVpnAppLink>()
          .toList();
      expect(links, hasLength(3));
      expect(links[0], isA<KingVpnGeoDataLink>());
      expect(links[0].name, 'a-domain');
      expect(links[1], isA<KingVpnGeoDataLink>());
      expect(links[1].name, 'z-domain');
      expect(links[2], isA<KingVpnConfigLink>());
    });
  });
  ...
  group('subscription links', () {
    test('does not export HWID or consent, including when enabled', () {
      ...
      final uri = KingVpnAppLinkGenerator.subscription(
      ...
      expect(KingVpnAppLinkParser.parse(uri), isA<KingVpnSubscriptionLink>());
    });
    test('normalizes a plain subscription URL', () {
      ...
      expect(uri, isNotNull);
      final link = KingVpnAppLinkParser.parse(uri!)! as KingVpnSubscriptionLink;
    ...
    test('rejects incomplete or unsupported age key pairs', () {
      expect(
        KingVpnAppLinkGenerator.subscription(
  ...
  test('generates and parses a GeoData link', () {
    final uri = KingVpnAppLinkGenerator.geoData(
      _geoData('Community Domain', url: 'https://example.com/community.dat'),
    );

    expect(uri, isNotNull);
    final link = KingVpnAppLinkParser.parse(uri!)! as KingVpnGeoDataLink;

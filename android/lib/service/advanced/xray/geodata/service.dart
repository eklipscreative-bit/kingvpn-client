...
import 'package:kingvpn/core/model/geo_dat.dart';
...
import 'package:kingvpn/gen/assets.gen.dart';
import 'package:kingvpn/service/shared/event_bus/service.dart';
import 'package:kingvpn/service/advanced/xray/geodata/download.dart';
import 'package:kingvpn/service/advanced/xray/geodata/model.dart';
import 'package:kingvpn/service/advanced/xray/geodata/system_state.dart';
import 'package:kingvpn/service/shared/in_flight_operations.dart';
import 'package:kingvpn/service/shared/command_serial_executor.dart';
...
class GeoDataService {
  ...
  Future<void> _recoverImportDrafts(
    ...
    await for (final entry in parent.list(followLinks: false)) {
      ...
      if (directoryName.startsWith('.kingvpn-geodata-backup-')) {
        if ((await _flatNames(entry)).isEmpty) await _deleteStage(entry);
        continue;
      }
      if (!directoryName.startsWith('.kingvpn-geodata-import-') ||
  ...
  Future<Directory> _newStage(String prefix) async {
    ...
    return parent.createTemp('.kingvpn-geodata-$prefix');
  }
  ...
  Future<void> _deleteStage(Directory stage) async {
    ...
    if (p.normalize(p.dirname(stage.path)) != expectedParent ||
        !p.basename(stage.path).startsWith('.kingvpn-geodata-')) {

  ...
  Future<LaunchAtLoginStatus> _enable() async {
    if (!File(_executable).existsSync()) {
      return const LaunchAtLoginStatus.error(
        'The KingVPN executable does not exist.',
      );
    }
    ...
    if (existingShortcut != null && !_canReplaceShortcut(existingShortcut)) {
      return const LaunchAtLoginStatus.error(
        'The KingVPN startup shortcut belongs to another executable.',
      );
    }
...
final class Win32WindowsLaunchAtLoginStore
  ...
  static const _shortcutName = 'KingVPN Client.lnk';
  ...
  void writeShortcut({
    ...
    _withSta(() {
      using((arena) {
        ...
        shellLink.setDescription(arena.pcwstr('KingVPN Client'));
